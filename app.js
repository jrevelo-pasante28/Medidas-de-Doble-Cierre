const API_URL =
  "https://script.google.com/macros/s/AKfycbzbu5ehCUrObC1gNev6tUp9VOXLBmfWRvAM9l-l3ag-pr7zs_2_2LsyVgCakt6mCOb4/exec";
...
function guardarEstado() {
  ...
  estado.visita.sustrato_tapa =
    document.getElementById("v-sustrato-tapa").value;
  estado.visita.sustrato_cuerpo =
    document.getElementById("v-sustrato-cuerpo")?.value || "";
  estado.visita.proveedor_tapa =
...
function cargarEstado() {
  ...
  try {
    ...
    delete estado.visita.producto;
    estado.visita.sustrato_tapa = estado.visita.sustrato_tapa || "";
    estado.visita.sustrato_cuerpo = estado.visita.sustrato_cuerpo || "";
    ...
    document.getElementById("v-unidad").value =
      estado.visita.unidad || "milimetros";
    document.getElementById("v-sustrato-tapa").value =
      estado.visita.sustrato_tapa || "";
    if (document.getElementById("v-sustrato-cuerpo")) {
      document.getElementById("v-sustrato-cuerpo").value =
        estado.visita.sustrato_cuerpo || "";
    }
    document.getElementById("v-proveedor-tapa").value =
...
async function cargarListasDesdeServidor() {
  ...
  statusDiv.innerHTML =
    '<i class="fa-solid fa-spinner fa-spin"></i> Cargando listas...';
  ...
  try {
    ...
    if (data.status === "success") {
      ...
      statusDiv.className =
        "text-xs font-bold bg-blue-900 px-3 py-1 rounded-full shadow-inner text-white";
    }
  } catch (error) {
    ...
    statusDiv.className =
      "text-xs font-bold bg-red-600 px-3 py-1 rounded-full shadow-inner text-white";
  }
}
...
function autoCompletarCodigoCliente() {
  const nombreCliente = document
    .getElementById("v-cliente")
    .value.toLowerCase();
  const clienteEncontrado = baseClientes.find(
    (c) => c.nombre.toLowerCase() === nombreCliente,
  );
...
function calcularTraslapesDeCabezal(infoIdx, cabIdx) {
  ...
  const espesorTapaIngresado =
    parseFloat(String(informe.espesor_tapa).replace(",", ".")) || 0;
  const espesorTapa =
    estado.visita.unidad === "pulgadas"
      ? espesorTapaIngresado / 25.4
      : espesorTapaIngresado;
  ...
  for (let x = 0; x < 3; x++) {
    ...
    if (gc > 0 && gt > 0 && ancho > 0 && espesorTapa > 0) {
      ...
      const inputTraslape = document.getElementById(
        `input_tras_${infoIdx}_${cabIdx}_${x}`,
      );
...
function actualizarCampoMedicion(infoIdx, cabIdx, campo, subIdx, valor) {
  if (subIdx !== null)
    estado.informes[infoIdx].mediciones[cabIdx][campo][subIdx] = valor;
...
function actualizarCampoInfo(infoIdx, campo, valor) {
  ...
  if (campo === "espesor_tapa") {
    estado.informes[infoIdx].mediciones.forEach((_, cIdx) =>
      calcularTraslapesDeCabezal(infoIdx, cIdx),
    );
  }
...
function actualizarUnidadMedida(valor) {
  ...
  estado.informes.forEach((informe, infoIdx) => {
    informe.mediciones.forEach((_, cabIdx) =>
      calcularTraslapesDeCabezal(infoIdx, cabIdx),
    );
  });
  guardarEstado();
...
function agregarCabezal(infoIdx) {
  estado.informes[infoIdx].mediciones.push({
    id_medicion: generarID(),
    num_cabezal: estado.informes[infoIdx].mediciones.length + 1,
    prof: ["", "", ""],
    esp: ["", "", ""],
    ancho: ["", "", ""],
    gc: ["", "", ""],
...
function eliminarCabezal(infoIdx, cabIdx) {
  ...
  estado.informes[infoIdx].mediciones.forEach(
    (m, i) => (m.num_cabezal = i + 1),
  );
  guardarEstado();
  renderizarInformes();
}
...
function renderizarFilaCabezal(i, c, m) {
  const triple = (lbl, cmp) => {
    const readonlyAttr =
      cmp === "tras"
        ? `readonly class="w-full border p-1 rounded text-xs text-center bg-slate-200 text-slate-500 cursor-not-allowed"`
        : `class="w-full border p-1 rounded text-xs text-center focus:ring-1 focus:ring-blue-400 outline-none"`;
    return `
        <div class="w-full">
...
function initFirma() {
  ...
  firmas.forEach((canvasActual) => {
    ...
    const firmaGuardada =
      canvasActual.id === "signature-pad-cliente"
...
function limpiarFirma(tipo = "cliente") {
  ...
  const canvasObjetivo =
    tipo === "tecnico" ? canvasTecnico : canvasCliente || canvasAntiguo;
...
async function enviarAlServidor() {
  ...
  try {
    ...
    const informesParaEnviar = (estado.informes || []).map((informe) => {
      const producto = String(informe.producto ?? "").trim();
      const linea = String(informe.linea ?? "").trim();
      const cerradora = String(informe.cerradora ?? informe.maquina ?? "").trim();
      const formatoCuerpo = String(informe.formato_cuerpo ?? "").trim();
      const formatoTapa = String(informe.formato_tapa ?? "").trim();
      const espesorTapa = String(informe.espesor_tapa ?? "").trim();
      const espesorCuerpo = String(informe.espesor_cuerpo ?? "").trim();
      ...
      return {
        ...
        cerradora,
        linea,
        formato_cuerpo: formatoCuerpo,
        formato_tapa: formatoTapa,
        espesor_tapa: espesorTapa,
        espesor_cuerpo: espesorCuerpo,
      };
    });

    const visitaParaEnviar = { ...estado.visita };
    delete visitaParaEnviar.producto;

    const datosParaEnviar = {
      ...estado,
      visita: visitaParaEnviar,
      informes: informesParaEnviar,
      firmaBase64: estado.firmaClienteBase64 || estado.firmaBase64,
      firmaClienteBase64: estado.firmaClienteBase64 || estado.firmaBase64,
    };
    ...
    btn.className =
      "text-xs font-bold bg-green-600 px-3 py-1 rounded-full shadow-inner text-white";
...
async function generarPDF() {
  ...
  try {
    ...
    const marginX = 8;
    const logoFadesaSrc = await cargarImagenComoDataURL(
      "Imagenes/LOGO2-02.png",
    );
    const logoTecnicoSrc = await obtenerLogoTecnicoPorNombre(
      estado.visita?.tecnico,
    );
    ...
    estado.informes.forEach((informe, idx) => {
      ...
      doc.setFontSize(14);
      doc.text(
        "Reporte Final de Evaluación de Doble Cierre",
        pageWidth / 2,
        20,
        { align: "center" },
      );
      ...
      campo("SUSTRATO CUERPO:", estado.visita?.sustrato_cuerpo, col2X, y, 31, 48);
      campo("PROVEEDOR TAPA:", estado.visita?.proveedor_tapa, col3X, y, 30, 48);

      const mediciones = Array.isArray(informe.mediciones)
        ? informe.mediciones
        : [];
      const head = [
        [
      ...
      const body =
        mediciones.length === 0
          ? [
              [
                {
                  content: "Sin mediciones registradas",
                  colSpan: 20,
      ...
      const firmaClienteSrc =
        typeof estado.firmaClienteBase64 === "string" &&
        estado.firmaClienteBase64.startsWith("data:image")
          ? estado.firmaClienteBase64
          : typeof estado.firmaBase64 === "string" &&
              estado.firmaBase64.startsWith("data:image")
            ? estado.firmaBase64
            : "";
      const firmaTecnicoSrc =
        typeof estado.firmaTecnicoBase64 === "string" &&
        estado.firmaTecnicoBase64.startsWith("data:image")
          ? estado.firmaTecnicoBase64
          : "";
      ...
      doc.setFontSize(9);
      doc.setFont("helvetica", "bold");
      doc.text("Firma del Cliente", pageWidth / 4, finalY + 27, {
        align: "center",
      });
      doc.text("Firma del Técnico", (pageWidth * 3) / 4, finalY + 27, {
        align: "center",
      });
      ...
      if (logoTecnicoSrc) {
        ...
        try {
          ...
          console.warn(
            "No se pudo insertar la firma del técnico en la esquina inferior derecha:",
            imgErr,
          );
        }
      }
    ...
    const cliente = String(estado.visita?.cliente || "General")
      .replace(/[\\/:*?"<>|]/g, "_")
      .trim();
    ...
    if (pdfButton) {
      ...
      pdfButton.innerHTML =
        '<i class="fa-solid fa-file-pdf mr-2"></i> Generar PDF(s)';
    }
  }
}
